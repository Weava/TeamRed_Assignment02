...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
		...
		[SerializeField] private GameObject m_WaypointSystem;
		...
		[SerializeField] private float m_MinWaypointDistance = 8.0f;
        ...
        private void Awake()
        {
			...
			m_Targets = new Transform[m_WaypointSystem.transform.childCount];
			int count = 0;

			foreach (Transform child in m_WaypointSystem.transform) {
				m_Targets [count] = child;
				count++;
			}

			Debug.Log (m_Targets [4].gameObject.name + " Name");

			Debug.Log (m_Targets.Length + " Waypoints");
